...
func _input(event: InputEvent) -> void:
    ...
    for device in allowed_device:
        _press_vector("move_backward", "move_forward", "move_right", "move_left", Vector2(Input.get_joy_axis(device, JOY_AXIS_LEFT_Y), Input.get_joy_axis(device, JOY_AXIS_LEFT_X)))
